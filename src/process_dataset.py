...
from utils_and_constants import delete_and_recreate_dir
...
def target_encode_categorical_features(df: pd.DataFrame, categorical_columns: list[str],
...
def impute_and_scale_data(df_features: pd.DataFrame) -> pd.DataFrame:
    ...
    x_processed = imputer.fit_transform(df_features.values)

    # Scale and fit with zero mean unit variance
    scaler = StandardScaler()
    x_processed = scaler.fit_transform(x_processed)

    return pd.DataFrame(x_processed, columns=df_features.columns)
...
def main():
    ...
    weather.to_csv(path_or_buf=PROCESSED_DATASET, index=None)
